...
<body>
    <div class="wrap">
        ...
        <div class="tabs" id="platformTabs">
            <button type="button" class="tab active" data-platform="apple">Apple فقط</button>
        ...
        <div class="grid">
            <div>
                ...
                <div class="panel" style="margin-top:16px">
                    ...
                    <p class="hint" style="margin-top:12px">الأختام تظهر تلقائياً (دوائر ملونة). ارفع PNG اختياري
                        لاستبدال شكل الختم المكتمل أو غير المكتمل.</p>
                    <div class="row2" style="margin-top:8px">
                        <div class="field">
                            <label>أيقونة الختم المكتمل (PNG)</label>
                            <input type="file" id="stampCompletedUpload" accept="image/png,.png">
                            <img id="stampCompletedPreview" class="thumb" style="display:none;margin-top:8px"
                                alt="">
                        </div>
                        <div class="field">
                            <label>أيقونة الختم غير المكتمل (PNG)</label>
                            <input type="file" id="stampEmptyUpload" accept="image/png,.png">
                            <img id="stampEmptyPreview" class="thumb" style="display:none;margin-top:8px"
                                alt="">
                        </div>
                    </div>
                </div>
                ...
                <div class="panel" style="margin-top:16px" id="appleSection">
                    ...
                    <div class="row2">
                        ...
                        <div class="field">
                            ...
                            <input type="number" id="apple_strip_overlay" min="0" max="1"
                                step="0.05">
                        </div>
                    </div>
                    <div class="row2">
                ...
                <div class="panel" style="margin-top:16px" id="googleSection">
                    ...
                    <div class="field">
                        <label>شفافية خلفية الشريط (0–1)</label>
                        <input type="number" id="google_strip_overlay" min="0" max="1"
                            step="0.05">
                    </div>
                </div>
                ...
                <div class="actions">
                    ...
                    <button type="button" class="btn btn-secondary" id="testGoogleBtn" style="display:none">تجربة
    ...
    <script>
        ...
        function render() {
            ...
            const logoHtml = state.logo_url ? `<img class="card-logo" src="${mediaUrl(state.logo_url)}" alt="">` : '';
    ...
    `).join('');
        ...
        async function uploadImage(input, kind) {
            ...
            if (!res.ok) {
                ...
                setStatus(err.message || 'فشل رفع الصورة.');
                return;
            }
            ...
            if (kind === 'logo') {
                ...
                img.onerror = () => setStatus('تعذّر عرض الشعار — تحقق من storage:link');
            } else if (kind === 'strip') {
                ...
                img.src = url;
                img.style.display = 'block';
                img.onerror = () => setStatus('تعذّر عرض خلفية الشريط — تحقق من storage:link');
                realStripUrls = {
                    apple: null,
                    google: null
                };
                ...
                img.src = url;
                img.style.display = 'block';
                img.onerror = () => setStatus('تعذّر عرض أيقونة الختم المكتمل — PNG فقط');
                realStripUrls = {
                    apple: null,
                    google: null
                };
                ...
                img.src = url;
                img.style.display = 'block';
                img.onerror = () => setStatus('تعذّر عرض أيقونة الختم غير المكتمل — PNG فقط');
                realStripUrls = {
                    apple: null,
                    google: null
                };
        ...
        $('previewBtn').addEventListener('click', async () => {
            ...
            setStatus(stripUrl ? `تم توليد شريط ${label}.` : `تعذر توليد شريط ${label} (تحقق من GD).`);
        });

        let lastGoogleSaveUrl = null;

        $('testGoogleBtn').addEventListener('click', async () => {
            setStatus('جاري توليد Google Save URL...');
            const res = await postJson(routes.preview);
            const data = await res.json();
            if (!data.google_configured) {
                setStatus('Google Wallet غير مهيأ — أضف GOOGLE_WALLET_ISSUER_ID و Service Account في .env');
                return;
            }
            if (!data.google_save_url) {
                setStatus(data.google_error ||
                    'تعذّر إنشاء Save URL — تأكد أن GOOGLE_WALLET_FALLBACK_LOGO يرجّع صورة PNG/JPG على HTTPS عام'
                    );
                return;
            }
            lastGoogleSaveUrl = data.google_save_url;
            window.open(data.google_save_url, '_blank', 'noopener');
            setStatus('تم فتح رابط Google Save URL.');
        });
